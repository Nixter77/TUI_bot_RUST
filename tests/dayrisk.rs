...
use tui_bot::dayrisk::{
    apply_day_risk, default_daily_loss_r, default_daily_loss_usdt, evaluate, one_r_usdt,
    r_budget_usdt, utc_day_key,
};
...
fn one_r_is_start_equity_times_risk_pct() {
    ...
    assert_eq!(
        r_budget_usdt(d("10000"), Decimal::ZERO, d("3")),
        Decimal::ZERO
    );
}
...
fn usdt_still_trips_even_when_r_budget_is_larger() {
    ...
    assert!(
        risk.halt,
        "USDT layer must still trip under a larger R budget (OR not max)"
    );
}
