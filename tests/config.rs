...
fn leverage_and_notional_options() {
    ...
    let unset = load_config(false, None, Some(&HashMap::new())).unwrap();
    assert_eq!(unset.s5_max_positions, unset.s4_max_positions);

    let mut env = HashMap::new();
    env.insert("STRATEGY4_MAX_POSITIONS".into(), "4".into());
    env.insert("STRATEGY5_MAX_POSITIONS".into(), "3".into());
    let split = load_config(false, None, Some(&env)).unwrap();
    assert_eq!(split.s4_max_positions, 4);
    assert_eq!(split.s5_max_positions, 3);

    let mut env = HashMap::new();
    env.insert("STRATEGY5_MAX_POSITIONS".into(), "0".into());
    assert!(load_config(false, None, Some(&env)).is_err());
}
