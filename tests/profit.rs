...
use tui_bot::profit::{
    account_profit, current_equity, load_persisted_starting_equity, persist_starting_equity,
    pin_starting_equity, EquityPin,
};

#[test]
fn profit_is_equity_minus_start() {
    let profit = account_profit(
        Decimal::from(10500),
        Decimal::new(2505, 1),
        Decimal::from(10000),
    );
...
fn rebasing_every_poll_hides_real_profit() {
    ...
    assert_eq!(
        account_profit(later_wallet, later_upnl, rebased),
        Decimal::ZERO
    );
...
fn persist_roundtrip() {
    ...
    assert_eq!(
        load_persisted_starting_equity(Some(&tmp.path().join("missing"))),
        None
    );
}
