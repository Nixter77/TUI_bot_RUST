...
* **Вселенная**: только **BTC / ETH / SOL**, зелёный 24h **+0.4%…+12%**. Дампы и TradFi отсекаются. У дневного хая мейджора не пропускаем — это тренд, не догон пыли.
* **Фильтры**: закрытая 5-минутная свеча должна быть зелёной; 4ч закрытие выше EMA20 (если история есть); если открытый слот уже в минусе — новый не открываем.
