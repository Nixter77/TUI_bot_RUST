...
| **В ожидании входа** | **книга стратегии**, не топ 24h. Для S4 — ликвидный откат (объём, не догон %). Причина, почему ещё не купил, и **до входа:** сколько осталось (время / % 24h / до VWAP). |
...
* **Вселенная**: только **BTC / ETH / SOL**, зелёный 24h **+0.4%…+12%** (сетап режет mid-band **[2%, 4%)** — short-horizon fade). Дампы и TradFi отсекаются. У дневного хая мейджора не пропускаем — это тренд, не догон пыли.
* **Фильтры**: закрытая 5-минутная свеча зелёная; 4ч выше EMA20; **3д** импульс ≥ +1% по 4h (fail-open без истории); догон 24h≥2.5% без 1h подтверждения — skip; слот в минусе — новый не открываем.
