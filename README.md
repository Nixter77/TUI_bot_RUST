...
* **Вселенная**: все торгуемые USDT-M с 24h объёмом (включая majors), без junk/1000x. Не три монеты. Книга по quote volume, cap 80 (1d klines). Слот live по-прежнему 1.
* **Сигнал на вход**: закрытие **дневной** свечи выше **40-дневного** максимума (Donchian 40) и close > **EMA100**. Зелёная свеча. ADX не режем — на held-out он ухудшал PF. Вход **только в первые 2 часа после 00:00 UTC** (закрытие той свечи). Днём по вчерашнему пробою не догоняем. Live last не ниже close и не дальше **+3%**. Стоп шире **15%** — skip.
...
* **Почему не 20/10 EMA50**: ~1500 дневных баров BTC/ETH/SOL, train 70% PF 2.59, **held-out 30% PF 0.62**. 40/20 + EMA100: train PF 2.47 / held PF 1.69. Это не live TestNet.
