...
| `src/trend.rs` | Стратегия 3 (Trend): пробой канала Дончяна (Donchian 40/20) на дневных свечах. |
...
| `--strategy 1\|2\|3\|4\|5` | опция | Выбор стартовой стратегии (по умолчанию `1`). `5` = S5 Verify, continuation на 1ч. |
...
| **В ожидании входа** | **книга стратегии**, не топ 24h. S4 — ликвидный откат; **S2 — majors BTC/ETH/SOL**; **S3 — ликвидный USDT-M стакан** (не три majors, не лента %). Причина + **до входа:** окно сессии / % / VWAP / закрытие свечи. Вне часов — время до следующего старта UTC, не «ждёт свечу». |
| **Топ роста / падения** | лента рынка 24h % (12 / 5). Это **не** список покупок; тег только если имя уже в позиции или в книге входа. |
...
# Старт со Скальпом (Стратегия 2) — BTC/ETH/SOL; окна STRATEGY2_* (ALWAYS_ENTER=1 → 24/7)
...
graph TD
    ...
    S1 --> |BTC/ETH/SOL 24h %| S1_Exec[SL/TP трейлинг + Корзина до N позиций]
    S2 --> |EMA9 / VWAP / RSI 5m| S2_Exec[Быстрый отскок BTC/ETH/SOL]
    S3 --> |Donchian 40/20 1d| S3_Exec[Дневной тренд + 8R цель]
...
Покупка **самого сильного за 24h** среди ликвидных мейджоров. Альты 24h (MORPHO/SPK/GIGGLE) сюда не входят — это стол Continuation (клавиша `4`).

* **Вселенная**: только **BTC / ETH / SOL**, зелёный 24h **+0.4%…+12%**. Дампы и TradFi отсекаются. У дневного хая мейджора не пропускаем — это тренд, не догон пыли. Mid-band **[2%, 4%)** и **3д импульс** сняты (90d walk: нож у 2% и train-mirage; на радаре резали ETH при живом 24h).
* **Фильтры**: закрытая 5-минутная свеча зелёная; 4ч выше EMA20; догон 24h≥2.5% без 1h подтверждения — skip; слот в минусе — новый не открываем.
* **Тайминг**: Окна UTC (`00–02`, `07–10`, `13–16`) либо 24/7 при `STRATEGY1_ALWAYS_ENTER=1`.
* **Выход**: стоп `TRAIL_PCT` (2.0%). До 1R **не** трейлим — ждём. На 1R в руке — fee-aware безубыток, потом только-вверх трейл. TP не уже **2R** стопа (`TAKE_PROFIT_PCT` default 5%; если в `.env` стоит 2.5% при стопе 2% — код поднимет до 4%). Красная 5м и выпадение из топа **не** закрывают лонг.
...
### 2. Scalp (Клавиша `2`)
Скальпинг на откатах к EMA9 / VWAP (5m). Книга = **BTC/ETH/SOL majors** (как S1) — **не** топ роста 24h. См. `SCALP_PLAN.md`.

* **Вселенная**: `BTCUSDT`, `ETHUSDT`, `SOLUSDT` (monitor «В ожидании» = эта книга, лента 24h отдельно).
* **Сигнал**: `EMA9 > EMA21`, цена выше VWAP, RSI в полосе, зелёная свеча + откат к EMA/VWAP. Без новых индикаторов до метрик.
* **Сессия**: `STRATEGY2_ENTRY_HOURS` / `STRATEGY2_ALWAYS_ENTER` (TUI передаёт `ScalpParams::from_config`). Вне окна — flatten «конец сессии».
* **Выход**: TP **2R** (fee-padded) · ATR SL · trail/fee-aware BE после 1R · **1.5R bank** · peak≥0.8R → замок 0.25R · `max_hold_bars` default **8** · reversal.
* **Слот**: 1 позиция. Live journal S2 пока **n=0** — offline BT ожидаемо −EV; не claim edge / не крутить `RISK_PCT` при PF_net≤1.

---

### 3. Trend (Клавиша `3`)
Позиционная трендовая стратегия на пробой дневного диапазона Дончяна (Turtle-style).

* **Вселенная**: ликвидный USDT-M. **BTC/ETH/SOL всегда в книге** (TestNet volume врёт и иначе хоронит majors). Остальные по quote volume, cap 80. **Не** TestNet-penny: цена ≥ 0.50 USDT, 24h ≤ +20%. Junk/1000x out. Слот live 1. BTC 4h bear/panic — новых входов нет.
* **Сигнал на вход**: закрытие **дневной** свечи выше **40-дневного** максимума (Donchian 40) и close > **EMA50**. Зелёная свеча. Если в один день несколько пробоев — берём **наименее растянутый** (только что вышел из канала), не тот у кого больше TestNet-объём. Вход **только в первые 2 часа после 00:00 UTC**. Live last не ниже close и не дальше **+3%**. Стоп шире **10%** — skip.
* **Таймфрейм**: `1d`. `cargo --backtest` для S3 идёт по **1d**, не по 5m.
* **Выход**: закрытие ниже Donchian **20** · стоп 2 ATR · chandelier 2.5 ATR от пика · TP-крышка **8R**. Live-выход/трейл несут `symbol` (раньше пустая строка ломала close при нескольких слотах).
* **Почему EMA50, не EMA100**: 25 ликвидных имён, ~1500×1d, 1 слот. Shipped 40/20 EMA100 на этой книге: held-out PF **0.86** pnl −1.53. 40/20 EMA50 + weakest: held n=13 WR 69% PF **2.71** pnl +7.92, walk-forward 3/3. Это **не** гарантия live TestNet (AIN/POWER были penny-pump на фейковом объёме).
...
* **Вселенная**: Топ-**20** по суточному объёму торгов (не по %, отсекаются неликвидные шиткоины с ценой < 0.5 USDT). Зелёный 24h в полосе **+0.5%…+35%** (дампы и мега-пампы out; stretch_pct только для дампов).
...
| `TAKE_PROFIT_PCT` | `0.050` | Тейк S1. Пол: **2R** стопа (`TRAIL_PCT×2`), даже если в `.env` стоит 2.5%. |
| `TRAIL_PCT` | `0.020` | Начальный стоп и шаг трейла после безубытка (2.0%). До 1R трейл не двигаем. |
| **Стратегия 2 (Scalp)** | | |
| `STRATEGY2_ALWAYS_ENTER`| `0` | `1` — входы 24/7 (игнор окон UTC). |
| `STRATEGY2_ENTRY_HOURS` | как S1 (`0-2,7-10,13-16`) | Окна UTC; вне окна — flatten открытого лонга. |
| `STRATEGY2_MAX_HOLD_BARS` | `8` | Тайм-стоп в барах сигнала 5m (1–240). |
